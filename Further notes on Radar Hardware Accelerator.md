...
### Dummy parameter set
  ...
  It is very typical that EDMA is used to move data into HWA local memory. One DMA is used to move the data, which automatically trigger another DMA that write a one hot signature (0x0001, 0x0002, 0x0004, 0x0008, and so on) into the DMA2ACCTRIG, thus triggering the HWA.
